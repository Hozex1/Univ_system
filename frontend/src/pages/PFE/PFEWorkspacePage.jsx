...
function StudentSubjectGallery({ subjects, loading, error, onSelect, selectedSubjectId, onRetry, myGroup, user }) {
  const [confirming, setConfirming] = useState(false);
  const validated = (Array.isArray(subjects) ? subjects : []).filter((s) => s.status === 'valide');
  
  const handleConfirm = async () => {
    if (!myGroup || !selectedSubjectId) return;
    const myMemberInfo = myGroup.groupMembers?.find(m => m.etudiantId === user?.etudiant?.id);
    if (myMemberInfo?.role !== 'chef_groupe') {
      alert('Only the group leader (chef_groupe) can confirm the subject.');
      return;
    }

    setConfirming(true);
    try {
      // 1. Add as the first wish (voeu)
      await request(`/api/v1/pfe/voeux/groupes/${myGroup.id}`, { 
        method: 'POST', 
        body: JSON.stringify({ sujetId: selectedSubjectId, ordre: 1 }) 
      });
      // 2. Trigger subject assignment
      await request(`/api/v1/pfe/groupes/${myGroup.id}/affecter-sujet`, { 
        method: 'POST', 
        body: JSON.stringify({ etudiantId: user?.etudiant?.id }) 
      });
      alert('Subject assigned successfully!');
      onRetry();
    } catch (err) {
      alert(err.message || 'Failed to confirm subject');
    } finally {
      setConfirming(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedSubjectId && validated.length > 0 && !myGroup?.sujetFinalId && (
        <div className="flex justify-end pt-4 border-t border-edge-subtle">
          <button
            type="button"
            onClick={handleConfirm}
            disabled={confirming || !myGroup}
            className="inline-flex items-center gap-2 rounded-xl bg-brand px-6 py-2.5 text-sm font-semibold text-surface transition-opacity hover:opacity-90 disabled:opacity-50"
          >
  ...
  const handleMemberChange = (index, field, value) => {
    setFormData(prev => {
      ...
      if (field === 'role' && value === 'chef_groupe') {
        newMembers.forEach((m, i) => { if (i !== index) m.role = 'membre'; });
      }
      ...
      <SectionHeader 
        ...
        action={isAdmin && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-brand px-4 py-2 text-sm font-medium text-surface shadow-sm transition-all hover:bg-brand-hover"
          >
            <Plus className="w-4 h-4" />
            Create Group
          </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-surface w-full max-w-lg rounded-2xl shadow-xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-5 overflow-y-auto">
              {submitError && (
                <div className="mb-4 p-3 bg-danger/10 border border-danger/30 text-danger rounded-xl text-sm font-medium">
                  {submitError}
                </div>
              )}
              <form id="create-group-form" onSubmit={handleSubmitGroup} className="space-y-4">
                <div>
                  ...
                  <input required type="text" value={formData.nom_ar} onChange={e => setFormData({...formData, nom_ar: e.target.value})} className="w-full rounded-xl border border-edge-subtle bg-control-bg px-3 py-2.5 text-sm text-ink outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-ink-secondary mb-1.5 uppercase">Group Name (English)</label>
                  <input type="text" value={formData.nom_en} onChange={e => setFormData({...formData, nom_en: e.target.value})} className="w-full rounded-xl border border-edge-subtle bg-control-bg px-3 py-2.5 text-sm text-ink outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-ink-secondary mb-1.5 uppercase">Co-encadrant (Teacher) *</label>
                  <select required value={formData.coEncadrantId} onChange={e => setFormData({...formData, coEncadrantId: e.target.value})} className="w-full rounded-xl border border-edge-subtle bg-control-bg px-3 py-2.5 text-sm text-ink outline-none focus:border-brand focus:ring-2 focus:ring-brand/20">
                    ...
                    {formData.members.map((m, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        {formData.members.length > 1 && (
                          <button type="button" onClick={() => handleRemoveMember(idx)} className="text-danger hover:text-danger/80 p-1">
                            <XCircle className="w-5 h-5" />
                          </button>
                        )}
                      </div>
            ...
            <div className="p-5 border-t border-edge-subtle flex justify-end gap-3 bg-surface-200/50 rounded-b-2xl">
              <button type="button" onClick={handleCloseModal} className="px-4 py-2 rounded-xl border border-edge bg-surface text-sm font-medium hover:bg-surface-200 transition-colors">
                Cancel
              </button>
              <button type="submit" form="create-group-form" disabled={submitting || loadingUsers} className="px-4 py-2 rounded-xl bg-brand text-surface text-sm font-medium transition-all hover:bg-brand-hover disabled:opacity-50 inline-flex items-center gap-2">
    ...
    if (activeTab === 'subjects') {
      ...
      return (
        <StudentSubjectGallery
          ...
          onRetry={retryActiveTab}
          myGroup={groups.find(g => g.groupMembers?.some(m => m.etudiantId === user?.etudiant?.id))}
          user={user}
        />
      );
    }
